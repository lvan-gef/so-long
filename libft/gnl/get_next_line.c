...
/*   Updated: 2023/03/02 16:48:12 by lvan-gef      ########   odam.nl         */
/*                                                                            */
/* ************************************************************************** */
...
static	char	*new_buffer(char *buffer, size_t start_pos)
{
	...
	if (new_len <= 0)
		return (gnl_free_it(NULL, buffer));
...
static	char	*append_buffer(char *buffer, char *str)
{
	...
	if (new_len <= 0)
		return (gnl_free_it(buffer, str));
	new_buf = gnl_calloc(new_len, sizeof(char));
	if (!new_buf)
		return (gnl_free_it(buffer, str));
	gnl_strjoin(new_buf, buffer, str, new_len);
	gnl_free_it(buffer, str);
	return (new_buf);
}
...
static char	*aline(char *buffer, size_t size)
{
	...
	if (!buffer)
		return (NULL);
	if (size <= 0)
		return (gnl_free_it(buffer, NULL));
...
static	char	*read_chunk(int fd, char *buffer)
{
	...
	while (!newline)
	{
		...
		if (read_size < 0)
			return (gnl_free_it(buffer, str_chunk));
		if (read_size == 0)
		{
			gnl_free_it(NULL, str_chunk);
			break ;
		}
...
char	*get_next_line(int fd)
{
	...
	if (newline_pos)
	{
		...
		if (line)
			...
			buffer = gnl_free_it(buffer, NULL);
		return (line);
	}
